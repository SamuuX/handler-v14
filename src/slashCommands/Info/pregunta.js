...
module.exports = {
   ...
   */
	async execute(client, interaction, prefix, GUILD_DATA) {
		...
		// .setColor("RANDOM")
			.setFooter({ text: `Encuesta realizada por ${interaction.user.tag}`, iconURL: interaction.user.displayAvatarURL({ dynamic: true }) })
			.setTimestamp();

		if (!interaction.member.permissions.has('MANAGE_MESSAGES')) return interaction.reply({ content: 'No tengo tienes permisos', ephemeral: true });
