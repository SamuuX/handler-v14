...
module.exports = {
    ...
    async  execute(client, interaction, prefix, GUILD_DATA) {
        ...
        let canal = client.channels.cache.get("1066925480474857615")

    if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.Administrator)) return interaction.reply({content: `❌ **Cjjaja! ¡Tienes que ser administrador para usar mi sistema de tickets!**`, ephemeral: true}).then(() => {
            setTimeout(() => {
